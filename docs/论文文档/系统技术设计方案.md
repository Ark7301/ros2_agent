...
- **SceneGraphValidator**：验证 LLM 生成的计划在场景图约束下是否可执行，包括空间约束验证和状态约束验证
...
场景图采用层级化语义环境表征，参考 SayPlan 的 3DSG 方案：

- **SceneNode**：场景图节点，包含 node_id、name、node_type（Building/Floor/Room/Object）、position、properties
- **SceneEdge**：场景图边，表示节点间关系（contains/on/in/near/connected_to）
- **SceneGraph**：完整场景图，支持子图提取（get_subgraph）、诱导子图（get_induced_subgraph，用于 EmbodiedRAG）、JSON 序列化
...
| 阶段 | 策略 | 目的 |
|---|---|---|
| Step 1：管道验证 | SceneGraphBuilder 从 JSON/YAML 加载预定义场景图（mock） | 跑通 Builder → WSM → Planner 数据管道 |
