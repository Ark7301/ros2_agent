...
| ROS2 集成 | 原生 | ROS2 Bridge 扩展（Humble + Jazzy） |
...
# 2. 确认 ROS2 Humble 已安装（Isaac Sim ROS2 Bridge 兼容 Humble 和 Jazzy）
ros2 --version

# 如果未安装 ROS2 Humble，参考官方文档：
...
Isaac Sim 内置的 ROS2 导航工作空间（`humble_ws`）包含：
...
Isaac Sim（PhysX 物理 + RTX 渲染）
    ...
    │       └── /amcl_pose（定位结果）
    │
    └── MOSAIC Agent 核心（Python asyncio）
            ├── NavigationCapability → Nav2 Action Client
            ├── SensorBridge → /scan, /odom, /camera
...
```python
# mosaic/nodes/ros2_bridge.py — ROS2 桥接节点管理
import asyncio
import threading
...
class ROS2BridgeManager:
    ...
    解决 MOSAIC asyncio 事件循环与 rclpy 回调机制的冲突。
    所有 ROS2 节点在 MultiThreadedExecutor 中运行，
    通过 asyncio.Future 桥接异步结果。
    """
...
Phase 1 需要在 `config/mosaic.yaml` 中新增 ROS2 和仿真相关配置：
...
2. **ROS2 环境必须在 Isaac Sim 之前 source**：Isaac Sim 的 ROS2 Bridge 在启动时检测 ROS2 环境。如果未 source，Bridge 扩展将无法加载。建议将 `source /opt/ros/humble/setup.bash` 写入 `~/.bashrc`。

3. **Python 版本冲突**：Isaac Sim 5.1 要求 Python 3.11，而 ROS2 Humble 默认使用系统 Python 3.10。解决方案：
   - Isaac Sim 在独立虚拟环境（Python 3.11）中运行
   - ROS2 节点使用系统 Python 3.10
   - 通过 ROS2 DDS 通信，不需要同一 Python 进程
