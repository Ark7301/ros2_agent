...
建议立项推进，但路线需要修正为 **`ARIA-centered, VLM-in-the-loop, atomic-orchestration, replanning-first, demo-driven`**。
...
> 在不依赖完整 ROS2 / SLAM / VLM 实机基础设施的前提下，交付一个由 ARIA 驱动、且 VLM 真实参与信息获取的基础具身智能决策编排 demo，使系统能够完成细粒度多步任务、状态确认、失败反馈和重规划，并具备清晰的演示可观测性。
...
也就是说，本阶段应采用“VLM 在环、基础设施降配”的方式推进。
...
1. `coffee_delivery`
说明：先通过 VLM 确认厨房场景和咖啡机/杯子存在，再导航到厨房、操作咖啡机、等待完成、拿起咖啡、送给用户

2. `towel_fetch`
说明：先通过 VLM 确认卧室场景和黄色毛巾位置，再导航、拿取、找到用户、递交

3. `blocked_route_replan`
说明：前往目标房间时发生临时阻塞，或到达后 VLM 观察结果与预期不一致，系统识别失败、更新上下文、重新规划替代路径并完成任务
...
- 后续接入真实 Nav2、机械臂、VLM、IoT 设备时，只替换原子能力实现，不重写编排逻辑。
...
flowchart TD
    ...
    TOOLS --> NAV[Navigation Mock]
    TOOLS --> MANI[Manipulation Mock]
    TOOLS --> APP[Appliance Mock]
    TOOLS --> WQ[World Query]

    VLM --> WORLD[Shared Demo World]
    NAV --> WORLD[Shared Demo World]
    MANI --> WORLD
    APP --> WORLD
    WORLD --> ARIA

    FAIL[Directed Failure / Runtime Feedback] --> ARIA
    ARIA --> REPLAN[Replanning]
    REPLAN --> LLM
```

这个阶段架构的本质不是“模拟硬件”，而是“模拟一个可推理、可更新、可失败、可恢复的具身世界”。
...
### Phase D：共享 Demo World

目标：

- 导航、操作、家电 stub 不再各自成功返回
- 所有 mock 行为必须受到共享世界状态约束
- 增加可控失败注入器，用于演示重规划

交付价值：

- 系统开始表现出具身智能，而不是简单工具调用
...
- 1 名工程支持负责 capability mock、demo config、runbook 与测试
...
2. VLM 是否真正参与了任务判断，而不是装饰性存在
3. 任务编排粒度是否已经收敛到未来原子能力方向
4. `mock-first` 路线是否符合当前资源和时间约束
5. ARIA 是否真正成为“中枢”，而不是概念包装
6. 重规划能力是否足以成为产品化差异点
7. 当前阶段交付是否能自然承接后续真实基础设施接入

## 12. 建议审查结论模板

可直接采用以下审查结论：

> 同意进入下一阶段开发，目标限定为“ARIA 驱动、VLM 在环、支持细粒度原子编排和失败后重规划的具身智能决策编排 demo”。本阶段不以真实 ROS2 / SLAM / VLM 全链路为完成标准，但要求 VLM 真实参与信息获取，优先交付可稳定复现、可解释、可演示、以重规划为差异点的大脑能力。真实基础设施集成作为后续阶段接口化接入。
